...
## Atlas Semantic Design & Provenance (2026-08-16, feat/atlas-provenance)

- **Design brief (ChatGPT)**: fuse Neo4j Bloom (interaction), Obsidian (simplicity), Kumu (cleanliness), GraphXR (exploration), Wikidata (semantic richness), Gephi (algorithms). Philosophy: "beautiful graph at 20 nodes, useful graph at 20,000 nodes". Research artifact: `packages/atlas/DESIGN-RESEARCH.md` (~21KB, 6-tool teardown; thesis = search-first scene explorer with 4 rungs: overview supernodes → search scene 20–80 → expand-by-relationship ≤300 → inspect panel; `supersedes` = contradiction edge reserved red; confidence→opacity, superseded→dash). Designer spec: shape=family / color=type / size=importance.
- **Phase 1 — visual language + click-to-expand** (`packages/atlas/src/renderer/` + `layout/`): 6 baked shape textures (`shapes.ts`, white at resolution 2, donut = draw circle then `.cut()` — PIXI v8 Graphics has NO `evenOdd` fill option); `theme.ts` gains `NodeTheme.shape` (`NodeShape` = circle|rounded-square|hexagon|diamond|page|donut), `edgeSemantics` map (`structural` vs `reference` vs `story` — story = decided/affects/implements/generated/changed), per-type entries (decision = diamond amber r7.5, agent = hexagon violet, agent_message = circle violet r3.5), 3-tier `lod.minZoom` (structure 0 / code 0.4 / leaf 1.0), label `typePriority` (decision 5, adr 4, person/file/class 3, folder/doc 2), `dur.expand: 180`. `pixi-renderer.ts`: per-shape textures, story edges drawn as tapered quads (0.7→2.1px) + arrowhead (tipLen 6, wing 3.5) when emphasized, selection proximity emphasis in explore mode (1-hop full 0.75/0.45, 2-hop half, else 0.05), spawn-at-parent grow-in animation, LOD visibility (`zoom >= getLodMinZoom(type)`), label priority from theme. `force-layout.ts`: relation-aware `linkDistance` (contains 40 / references+contributes_to 60 / story 100 / default 70), `forceChainStraighten()` (0.02 pull of story-chain 2-hop middles onto the a→b segment) in the refine phase.
- **Bloom-style traversal** (`App.tsx`): click node → `mergeGraph(1-hop subgraph)` (dedupe by id/edge-key; graph grows, never re-renders the world) → focus mode with highlightIds → `flyToNode` → breadcrumb pushed (dedupe, cap 8). Breadcrumb nav below header (`.atlas-breadcrumbs`, top:48px/30px/z-19) with `handleBreadcrumbJump(i)` truncate + re-focus, clear button resets to explore + default layout.
- **Phase 2 — "Why" provenance drawer**: `GET /api/akg/decision?id=` (`packages/cli/src/commands/atlas.ts`, inside the `/api/akg/` handler, explicit columns only) returns `{ node, isDecision, who, affectedFiles, commits, relatedDecisions, tasks, conversation, confidence }`. **Evidence ladder**: edge-first (`decided`/`contributes_to`/`created_by` in-edges from person/agent; `documents`/`affects`/`implements`/`changed`/`generated`/`references` out-edges to files), then honest ±14-day temporal fallbacks — authors whose created/owned files were last-modified in-window (`author-near-date`), files `modified-near-date`, `git log --since/--until` commits (execSync, 5s timeout, silent on failure — window may be empty for pre-squash history), all other adr nodes as earlier/later, mesh journal `agent_message` rows in-window. Confidence = weighted sum (content .2, who .25, files .2, commits .15, conversation .1, related .1), capped 1. **Absence is reported, never invented** — empty sections render "no evidence" hints; a decision logged without edges (the current 7 `decision::` nodes have ZERO graph edges) still gets a confident story only when temporal evidence exists (e.g. "License keys: astlk_" scores 0.85 with 2 authors + 5 files + 3 mesh messages).
- **Client**: `DecisionProvenance` + `getDecisionProvenance(id)` in `akg-client.ts`; `components/ProvenancePanel.tsx` (hero with content + date + cluster, confidence meter, chips for who/files/related decisions that navigate the graph, commit list, conversation messages). `App.tsx`: drawer shows **Inspector | Why tabs** for `adr`/`decision::` selections (provenance prefetched on selection, loading spinner, resets to Inspector for non-decision nodes). CSS: `.drawer-tabs`, `.provenance-*` in atlas.css.
- **Gotchas**: `AkgNode.sourceFile` is `string | undefined` — null-coalesce (`?? null`) when mapping to a `string | null` field in server responses (TS2322). The SPA fallback swallows unknown paths — verify new `/api/akg/*` routes against the running serve, not the file server. Serve on Windows dies silently if launched with redirects/hidden window — use detached `Start-Process`, pid in `%TEMP%\opencode\atlas-serve.pid`.
- **Phase 3 — scale/LOD polish (2026-08-16, merged in PR #51)**: explore mode renders only the top `exploreSetCap` (200) most-important nodes (`typePriority × (1 + degree/10)`; selection/hover always promoted) so overview stays clean at thousands of nodes; label budget scales with zoom (`budgetPerZoom 40 × zoom`, cap `budgetMax` 160); refine phase adds `forceRepoOrbit()` (repos settle on a 340px shell ring around the workspace root, alphabetical angular slots, pinned nodes skipped). **Commit-synthesis hardening**: windowed git log capped at 8; keyword fallback (label+content words ≥4 chars, ≤6, stoplisted) drops generic keywords hitting >50% of the last 60 subjects (e.g. repo name "astrivya") and ranks matches by distinct-keyword hits, capped at 5. **Windows git gotcha**: `execSync` with `git log --format=%H|%aI|%an|%s` mangles `%a`/`%H` as cmd batch variables on Windows — windowed git log silently returned NOTHING until switched to `execFileSync("git", [...args])` (no shell). Decision nodes created recently (within ±14d) legitimately return 8 windowed commits (e.g. "Git canonical remote" created 2026-08-13 → 8 commits, confidence 1.0); July decisions get keyword fallback (0.45).
- **Phase 4 — rung-0 supernode overview (2026-08-16, PR #52)**: "Overview" mode chip (Globe icon) aggregates the whole graph into community supernodes — discs at member centroids, radius `min(maxR, baseR·√count)` (7→64px), tinted by the community's dominant type, cross-community edges collapsed into thin bundles (alpha scales with `log10(bundle size)`), labels only on the top `labelCap` (30) communities. Supernode layer lives in `supernodeContainer` (zIndex 2.5, between ring and labels); picking hits discs with a `hitRadius` floor; click → `onSupernodeSelect` → App drills into the community by focusing its top-priority member (Bloom 1-hop merge + breadcrumb). Per-frame centroid tracking runs only while `!settled` (O(N) single pass); `communityOf` lookups use a prebuilt `nodeCommunity` map (never `nodesArray.find` per edge). `updateNodesPositions`/`drawEdges`/`updateGraph`/`refreshLabels`/`updateRing` all short-circuit in overview mode. Server: `/api/akg/communities` returns `{ id, label, nodeCount, typeHistogram, dominantType }` from one GROUP BY (was DISTINCT + per-community COUNT). **Data note**: `nodes.community` holds ~100 cluster ids while the `communities` table holds 19 rows (pre-existing split) — stats.communities=19 ≠ /communities.length=100.
