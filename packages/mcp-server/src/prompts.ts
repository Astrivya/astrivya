...
export const PROMPT_DEFINITIONS: PromptDef[] = [
  ...
  },
  {
    ...
    arguments: [
      ...
      { name: "decision", description: "A decision taken during the task, if any (e.g. 'Adopted RRF over concatenation for search fusion')." },
...
export function buildPromptPrompt(name: string, args: Record<string, unknown> | undefined): string {
  switch (name) {
    ...
    case "decision_required": {
      ...
      const fileContext = String(args?.file_context ?? "").trim();
      const lines = [
        `A decision is required. Record it for the team: "${title}".`,
        "",
        `Reasoning: ${reasoning}`,
      ];
