...
import { getStorage, handleReadResource, handleToolCall, refreshContextDigest, setAkgStorage, setToolPlugins } from "./handlers";
...
function startWorkspaceWatcher(storage: AkgStorage, workspacePath: string): Watcher {
  ...
  const watcher = new Watcher(
    ...
    {
      onError: (err) => console.error(`[Astrivya MCP] File watcher error: ${err instanceof Error ? err.message : String(err)}`),
    },
  );
